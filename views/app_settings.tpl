<div class="container">
  ...
  <div class="row">
    <div class="col col-xs-12">
      <form data-zone="app-settings" name="settings"></form>
...
<script>
  ...
  // Render settings
  $.get('/settings/app.json').then(settings => Promise.resolve($("[data-zone='app-settings']").html(Handlebars.compile($('#template-settings').html())({ settings: settings.sort((a, b) => ([a.name, b.name]).sort().indexOf(a.name)) }))))
    .then((e) => $("button[type='submit']").removeAttr('disabled'));
  ...
  $("button[type='submit']").on('click', (e) => {
    ...
    $.ajax({ type: 'POST', url: '/settings/app', data: JSON.stringify(settings), dataType: 'json', contentType: 'application/json'})
      .then(() => Promise.resolve($(e.target).button('reset')))
