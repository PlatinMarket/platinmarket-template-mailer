...
function buildImageAddress(path) {
  return config.host + "/image" + path;
}

// Handlebars helpers
function registerHelpers(forPreview, attachments) {
  // Attach files
  handlebars.registerHelper('attach', (path) => {
    if (forPreview) return buildImageAddress(path);
    attachments.push(path);
    return "cid:" + surl(path, {separator: '_', lang: 'tr'});
  });

  // Image address
  handlebars.registerHelper('image', (path) => {
    return new Promise((resolve, reject) => {
      files.resolvePath({ path }).then(u => resolve(u.url.replace('?dl=0', '?raw=1'))).catch(err => {
        console.log(err);
        resolve("HATA");
      });
    });
...
Template.prototype.setStorageService = function (storageService) {
  this.storageService = files = storageService;
};
      ...
      try {
        ...
        handlebars.compile(template.subject || "")(Object.assign(templateParams, extendData))
          ...
          .then(_text => {
            ...
            resolve({ subject, html, text, template_id: template.folder });
          })
...
Template.prototype.get = function(name) {
    return new Promise((resolve, reject) => {
        setTimeout(() => {
            ...
            resolve(Object.assign({ id: name, type: Template.prototype.EMAIL }, templateConfig, this.getTemplate(name, templateConfig.textFallback), {folder: name}));
        }, 0);
    });
};
