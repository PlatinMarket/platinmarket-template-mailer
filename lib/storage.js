...
Storage.prototype.publicUrl = function (path) {
  path = this.sanitize(path);
  return this.makePublic(path).then(() => Promise.resolve(`https://storage.googleapis.com/${process.env.BUCKET}/${path}`));
};
...
Storage.prototype.directUpload = function (req, path) {
  ...
  return Promise.all(files.map(f => {
    return new Promise((resolve, reject) => {
      ...
      const stream = this.bucket.file(filePath).createWriteStream({ metadata: { contentType: f.mimetype } });
      stream.on('error', (err) => reject(err));
      stream.on('finish', () => this.bucket.file(filePath).getMetadata().then(m => resolve(m[0])).catch(err => reject(err)));
      stream.end(f.buffer);
    });
  }));
};
