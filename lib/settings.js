...
// OpenSSL EVP_BytesToKey implementation for backward compatibility with crypto.createCipher
...
function encrypt(text){
  if (!text) return text;
  const { key, iv } = evpBytesToKey(defaultSettings.auth_secret, 32, 16);
...
Settings.prototype.saveDefaults = function(settings) {
  return this.write('default', settings)
    ...
    .then(settings => {
      ...
      logging.log('verbose', 'Settings changed', defaultSettings);
...
s.read('default').then(settings => {
  defaultSettings = settings;
  logging.log('verbose', 'Settings read from datastore', defaultSettings);
