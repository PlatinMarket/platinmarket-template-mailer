...
Settings.prototype.save = function(user, data) {
  ...
  return this.create(user)
    ...
    .then(settings => {
      ...
      return datastore.save({ key: datastore.key(['User', this.id(newUser)]), data: newUser}).then(() => Promise.resolve(newUser)).catch(err => reject(err));
    });
};
...
Settings.prototype.get = function (user) {
  if (!user) return Promise.resolve(null);
  return datastore.get(datastore.key(['User', this.id(user)])).then(entity => {
...
Settings.prototype.users = function () {
  const query = datastore.createQuery('User').limit(100);
...
Settings.prototype.exists = function(user) {
  return datastore.get(datastore.key(['User', this.id(user)])).then(data => Promise.resolve(data && data.length > 0)).catch(err => Promise.resolve(false));
};
