...
module.exports = function (email, password) {
    return new Promise((resolve, reject) => {
        settings.read('auth')
          .then(auth => {
            if (!auth) return Promise.reject(new Error('Missing auth settings'));
            var payload = new Buffer(email + auth.secret.toString() + password).toString('base64');
            var url = auth.url.toString().replace('#payload#', payload);
