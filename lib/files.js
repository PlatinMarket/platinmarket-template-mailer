...
var drivers = [
  {
    ...
    downloadFile: (args) => {
      ...
      if (!args.path) return Promise.reject(new Error("Path not found"));
      args.path = config.bucket + args.path;
      return client.filesDownload(args);
    },
    uploadFile: (args) => {
      args = args || {};
      if (!args.path) return Promise.reject(new Error("Path not found"));
...
function Files(){
  ...
  if (this.ready) {
    mkdirp(config.cache_folder).catch(err => console.warn(err));
...
Files.prototype.downloadFile = function (args) {
  return new Promise((resolve, reject) => {
    ...
    if (fs.existsSync(cacheFile)) return resolve(cacheFile);
    this.driver.downloadFile(args).then(f => {
      try {
        fs.writeFile(cacheFile, f.fileBinary, 'binary', (err) => {
