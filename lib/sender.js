...
// Sender options
options.sender = Object.assign({
  email: '../sender/email'
}, options.sender || {});

// Call all sender
options.sender = Object.keys(options.sender).map(s => {return { name: s, func: require(options.sender[s]) }; });

// Queue Builder
function createQueue(sender) {
  var _queue = new Queue(sender.name, options.redis.port, options.redis.host, { password: options.redis.password });
...
Sender.prototype.getReady = function () {
  return new Promise((resolve, reject) => {
    try {
      this.queues = options.sender.map(s => createQueue(s));
      resolve();
    } catch (err) {
      reject(err);
    }
