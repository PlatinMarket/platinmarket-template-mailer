...
module.exports = (function() {
  ...
  router.use(['/send/:id', '/send'], (req, res, next) => {
    ...
    sender.getJobFromGUID(req.query.guid).then(jobs => {
      ...
      logging.log('error', 'SEND_PROCESS_0', err);
    ...
    templateStore.list()
      .then(templates => {
        ...
        Promise.all(templates.map(t => templateStore.get(t.id)))
          ...
          .catch(err => {
            logging.log('error', 'SEND_PROCESS_1_0', err);
            res.sendStatus(500);
          })
      })
      .catch(err => {
        logging.log('error', 'SEND_PROCESS_1_1', err);
        res.sendStatus(500);
      });
  });
  ...
  router.use(['/send/:id', '/send'], (req, res, next) => {
    ...
    settings.users()
      .then(users => {
        ...
        if (!req.user) {
          logging.log('warn', 'SEND_PROCESS_2_0', new Error('User ' + params.from + ' not found'));
          res.sendStatus(400);
        };
        next();
      })
      .catch(err => {
        logging.log('error', 'SEND_PROCESS_2_1', err);
        res.sendStatus(500);
      });
  });
  ...
  router.use(['/send/:id', '/send'], (req, res, next) => {
    Promise.all(req.templates.map(t => templateStore.render(t, req.body, { user: {name: req.user.name, email: req.user.email} })))
      ...
      .catch(err => {
        logging.log('error', 'SEND_PROCESS_3', err);
        res.sendStatus(500);
      });
  });
  ...
  router.use(['/send/:id', '/send'], (req, res) => {
    Promise.all(req.templates.map(t => sender.addQueue(t.template.type, t, req.user, req.body.to, (req.body.guid || null)))).then((jobs) => {
      ...
      logging.log('error', 'SEND_PROCESS_4', err);
      res.sendStatus(500);
    });
  });
